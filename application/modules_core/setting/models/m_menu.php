...
class m_menu extends CI_Model {
    ...
    function get_all_menu($params) {
        $sql = "SELECT * FROM my_menu WHERE parent_id = 0 AND portal_id = ? ORDER BY menu_order ASC";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $x = 0;
            foreach ($query->result_array() as $data) {
                ...
                $sql = "SELECT * FROM my_menu WHERE parent_id = ? ORDER BY menu_order ASC";
    ...
    function get_all_menu_by_portal_role($params) {
        $sql = "SELECT a.menu_id, a.portal_id, a.parent_id, a.menu_name, a.menu_url, a.menu_order, b.role_id, b.permission
            FROM my_menu a
            LEFT JOIN my_permission b ON b.menu_id = a.menu_id AND b.role_id = ?
    ...
    function get_menu_by_slug($params) {
        $sql = "SELECT * FROM my_menu WHERE menu_slug = ?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
    ...
    function add_menu($params) {
        $sql = "INSERT INTO my_menu(portal_id, parent_id, menu_name, menu_slug, menu_desc, menu_url, menu_order, menu_icon, creator, dc) VALUES(?,?,?,?,?,?,?,?,?,NOW())";
        return $this->db->query($sql, $params);
    }

    // edit menu
    function edit_menu($params) {
        $sql = "UPDATE my_menu SET parent_id = ?, menu_name = ?, menu_slug = ?, menu_desc = ?, menu_url = ?, menu_order = ?, menu_icon = ?, du = NOW() WHERE menu_id = ?";
        return $this->db->query($sql, $params);
    }

    // delete menu
    function delete_menu($params) {
        $sql = "DELETE FROM my_menu WHERE menu_id = ?";
        return $this->db->query($sql, $params);
    }
